...
Course.prototype.clearPreviousLesson = function()
{
	if(course.previous)
	{
		var prevLes = course.previous;
		setTimeout("eng.jQuery('.lesson"+prevLes+" .course-video-cont').html('');", 3000);
	}
}
...
Course.prototype.init = function()
{
	eng.jQuery('.course-lesson a').unbind('click').click(function()
	{
		...
		if(course.currentActivity)
			course.stopCurrentVideo(course.currentActivity - 2);
	...
	eng.jQuery('.course-prev-lesson').click(function()
	{
		course.currentActivity--;
		course.updateCurrentLink();
		course.stopCurrentVideo(course.currentActivity);
	...
	{
		window.location.href = domainUrl+'payment/';
	...
	if(course.resume != 0 && eng.jQuery('#lesson'+course.resume+'link').length)
	{
...
Course.prototype.stopCurrentVideo = function(ele)
{
	if(eng.jQuery('.lesson'+course.current+':eq('+(ele)+')').children('.course-video-cont').children('iframe')[0] && course.current != 0)
		eng.jQuery('.lesson'+course.current+':eq('+(ele)+')').children('.course-video-cont').children('iframe')[0].contentWindow.stopVideo();
}
...
Course.prototype.updateCourseNotes = function()
{
	...
	listClose.className = 'close-note';
	var textClose = document.createTextNode('x');
	listClose.appendChild(textClose);
	list.appendChild(listClose);
	eng.jQuery('#course-notes').append(list);
	eng.jQuery('#note').val('');
	course.updateUserNotes();
	return false;
}
