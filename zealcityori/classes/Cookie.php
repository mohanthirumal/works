...
class	Cookie
{
	...
	{
		$this->_content = array();
		$this->_expire = isset($expire) ? (int)($expire) : (time() + 60*60*24*30);
		$this->_name = md5($name);
		$this->_path = '/';
		...
		$this->_cipherTool = new Rijndael($this->_key, $this->_iv);
		$this->update();
	}
	...
	public function isLogged($withGuest = false)
	{
		/* Customer is valid only if it can be load and if cookie password is the same as database one */
	 	if ($this->logged == 1 AND $this->user_id AND Validate::isUnsignedId($this->user_id) AND User::checkPassword((int)($this->user_id), $this->password))
        	return true;
        return false;
	}
