...
zeal.extend({
	...
	userteam: {	
		selectTournament1: function (user_id, id, type, mid){
			...
			$.get(
			url,
			({'user_id': user_id, 'tour_id':id, 'type':type, 'mid':mid}),
			function(data)
			{						
		...
		dynamicScore: function(id, type, mid){
			var	url = base_url+'dynamic-score.php';		
			$.get(
			url,
			({'id': id, 'type':type, 'mid':mid}),
			function(data)
			{
				zeal.jQuery('#myteam1').html(data);
				setTimeout("zeal.userteam.dynamicScore("+id+", "+type+", "+mid+");", 30000);
			});			
		},
		dynamicResult: function(id, type, mid){
			...
			function(data)
			{
				...
				setTimeout("zeal.userteam.dynamicResult("+id+", "+type+", "+mid+");", 30000);
			});			
		}
	},
	tournament: {
		...
		fbEnabled: false,
		paginationStart: 16,
		getOlderTournaments: function()
		{
			zeal.jQuery('.tournament-loader').show();
			zeal.jQuery('.my-tournament-more-link').hide();
			var	url = base_url+'my-tournament-pagination.php';
			$.get(
				url,
				({'start': zeal.tournament.paginationStart}),
				function(data)
				{
					zeal.jQuery('#my-tournament').append(data);
					zeal.tournament.paginationStart = zeal.tournament.paginationStart + 15;
					zeal.jQuery('.tournament-loader').hide();
					if(data.length > 0)
						zeal.jQuery('.my-tournament-more-link').show();
				});
		},
		checkUserLike: function(id, type)
		{
			zeal.jQuery('#join-tournament').html('<div class="loading"></div>');
			zeal.jQuery('.transparent-container').fadeIn('slow');
			zeal.jQuery('#join-tournament').fadeIn('slow');
			zeal.tournament.id = id;
			zeal.tournament.type = type;
			setTimeout("zeal.tournament.fbTimeOutChecker();", 5000);
			FB.getLoginStatus(function(response) {
				zeal.tournament.fbEnabled = true;
				if (response.status === 'connected')
				{
					  var user_id = response.authResponse.userID;
					  zeal.tournament.getPageLikeState(user_id);
				}
				else
				{
		...
		fbTimeOutChecker: function()
		{
			if(!zeal.tournament.fbEnabled)
				alert('Sorry, you need facebook to join this free tournament');
		},
		getPageLikeState: function(user_id)
		{
			var page_id = "190533467632448";
			var fql_query = "SELECT uid FROM page_fan WHERE page_id = "+page_id+"and uid="+user_id;
			var the_query = FB.Data.query(fql_query);
			the_query.wait(function(rows) {
				if (rows.length == 1 && rows[0].uid == user_id)
				  zeal.tournament.showLikePage(2);
				else
					zeal.tournament.showLikePage(1);
			});
		},
		fbJoinShare: function()
		{
			FB.ui({
			  method: 'feed',
			  link: 'http://www.zealcity.com',
			  picture: 'http://www.zealcity.com/images/share-logo-friends.jpg',
			  name: 'Play and win real cash prizes',
			  caption: 'www.zealcity.com',
			  description: 'I created my team at captain of captains. Create your teams and challenge me to  win real cash only at zealcity.com.'
			}, function(response){
				if(!response)
					return false;
				zeal.jQuery('.fb-like-cont').hide();
				zeal.tournament.joinTournament(zeal.tournament.id, zeal.tournament.type);
			});
		},
		facebookShare: function(title, desc)
		{
			FB.ui({		 
				method: 'feed',
				picture: 'http://www.zealcity.com/images/share-logo-friends.jpg',
		...
		likeJoinTournament: function(id){
			zeal.jQuery('.fb-like-cont').hide();
			zeal.jQuery('#likeus-loader').show();
			zeal.tournament.joinTournament(zeal.tournament.id, zeal.tournament.type);
		},
		joinTourMore: function (){
			zeal.jQuery('.match-list').hide();
			zeal.jQuery('.match-det').removeClass('hide');
		},
		playerList: function(){
			zeal.jQuery('.player-list').hide();
			zeal.jQuery('.play-det').removeClass('hide');
		...
		selectTournament: function (tour_id, type){
			if(!loggedIn){
				zeal.jQuery('.join-tournament').hide();
		...
		inSufficientCashWindow: function(){
			zeal.jQuery('.jointournamentContainnor').hide();
			zeal.jQuery('.insufficientcash').fadeIn('slow');
		},
		confirmJoin: function (tour_id, type, ele){
			zeal.jQuery('.loading-tour-join').show();
			...
			zeal.tournament.joinTournament(tour_id, type);
		},
		...
		joinTournament: function (tour_id, type){
			var	url = 'join-tournament.php';
			$.ajax({
				type: 'GET',
				...
				cache: false,
				dataType : "json",
				data: 'action=join&tour_id=' + tour_id + '&type='+type,
				success: function(jsonData)
				{
					if(jsonData.hasErrors)
					{
						...
						if(jsonData.type == 1)
							window.location.href = 'my-tournament/'+jsonData.type+'-type/'+jsonData.id+'/0';
						else
							window.location.href = 'my-period-tournament-details/'+jsonData.type+'-type/'+jsonData.id;
				},
				error: function(XMLHttpRequest, textStatus, errorThrown) {
					// in front-office, do not display technical error
					//alert("TECHNICAL ERROR: unable to perform.\n\nDetails:\nError thrown: " + XMLHttpRequest + "\n" + 'Text status: ' + textStatus);
		...
		nextMatchTime: function (year, month, day, hour, minute, second){
			var austDay = new Date(year, month - 1, day, hour, minute, second);
			$('#next-starts-in').countdown({until: austDay, format: 'HMS',description: '', serverSync: zeal.tournament.matchServerTime, compact: true, expiryUrl: window.location.href});
		},
		periodTourTime: function (year, month, day, hour, minute, second){
			var austDay = new Date(year, month - 1, day, hour, minute, second);
			$('.period-tournament-timer').countdown({until: austDay, format: 'HMS',description: '', serverSync: zeal.tournament.matchServerTime});
		},
		activeTournament: 'tour-lobby-room',
		showTournament: function (parent, common, active, tab){
			...
			if(!tab)
				zeal.jQuery('.'+active+'-li').addClass('active');
		},
		showJoinTournamentTab: function (parent, common, active, tab){
			zeal.jQuery('.'+parent+' .'+common).hide();
			zeal.jQuery('.'+parent+' .'+active).show();
			zeal.jQuery('.'+parent+' ul li').removeClass('active');
			zeal.jQuery(tab).addClass('active');
		...
		showTournamentDetails: function (parent, common, active, tab){
			zeal.tournament.activeTournament = active;
			zeal.jQuery('.'+parent+' .'+common).hide();
			zeal.jQuery('.'+parent+' .'+active).show();
			zeal.jQuery('.'+parent+' .tournamentInfoClassHeader .headermenuClass').removeClass('active');
			zeal.jQuery(tab).addClass('active');
		},
		serverTime: function (){
	...
	user: {
		...
		closeSignin: function (){
			zeal.jQuery('.transparent-container').hide();
			//zeal.jQuery('.login-box-container').hide();
			zeal.jQuery('.join-tournament').hide();
		},
		signupconfirmPass: function(){
			...
			if((pass.length==0) && (conpass.length==0)){
				zeal.jQuery('#txtpassword').parent().addClass('txt-login-input-error');
				zeal.jQuery('#txtpassword2').parent().addClass('txt-login-input-error');
				temp=false;
			}
			...
			if(temp==false){
				...
				zeal.jQuery('#txtpassword').parent().removeClass('txt-login-input-error');
				zeal.jQuery('#txtpassword2').parent().removeClass('txt-login-input-error');
		...
		validateUsername: function(ele){
			...
			if(ele.value.length==0){
				usertemp=false;
				zeal.jQuery('#txtusernamesignup').parent().addClass('txt-login-input-error');
			}
			...
			zeal.user.unameerrorIndicate=true;
				...
				zeal.jQuery('#txtusernamesignup').parent().removeClass('txt-login-input-error');
...
//				zeal.jQuery('.email-ok-image').hide();	
				zeal.jQuery('#email').parent().addClass('txt-login-input-error');		
				return false;
			}
...
//					zeal.jQuery('.email-no-image').show();
					...
					zeal.jQuery('#email').parent().removeClass('txt-login-input-error');
...
//					zeal.jQuery('.email-no-image').hide();
		...
		showSignupSuccess: function(){
			zeal.jQuery('#signup').remove();
			zeal.jQuery('.transparent-container').show();
			zeal.jQuery('.loading').hide();
			zeal.jQuery('.login-show-container').hide();
	...
	facebook:{
		...
		signup: function() {
			...
			zeal.jQuery('.login-signup-btn').hide();
			zeal.jQuery('.loading').show();
			zeal.facebook.emailExist(function(returnValue) {
			 	if(returnValue)
				{					
					var username = zeal.jQuery('#txtusernamesignup').val();
					var password = zeal.jQuery('#txtpassword2').val();
					var captcha = zeal.jQuery('#verify-txt').val();
					zeal.jQuery.ajax({
						type: 'POST',  
						url: 'index.php',
						dataType : "json",
						...
						success: function(jsonData, textStatus, XMLHttpRequest){
							zeal.jQuery('.login-signup-btn').show();
							zeal.jQuery('.loading').hide();
							if(jsonData.hasErrors)
									zeal.errors.showError(jsonData.errors);
							else
							{
								zeal.errors.showError('Signup successfull');
								window.location.href = '?fblogin';
							}
						},  
...
//			var idname = document.getElementById('txtonlinepanno').value;
//			if(amt.length == 0)
//			{
...
//			
//		},
		withdrawamt1: function(type){
			if(type == 1)
			{
				var onlinwithproof = document.getElementById('proof-select').value;
				var onlineidname = document.getElementById('txtonlinepanno').value;
				if(onlinwithproof == 0)
				{
					alert("please select any proof");
					return false;
				}
				else if(onlineidname == '')
				{
					alert("please enter proof number");
					return false;
				}
				else
			...
			{
				var onlinwithproof = document.getElementById('chqproof-select').value;
				var onlineidname = document.getElementById('txtonlinepanno1').value;
				if(onlinwithproof == 0)
				{
					alert("please select any proof");
					return false;
		...
		cancelCreate: function(){
			zeal.jQuery('.transparent-container').hide();
			zeal.jQuery('.online-iddet').hide();
			zeal.jQuery('.my-account-maincontainer').show();
		},
		withdrawtype: function(type){
			var wType = type.value;
			if(wType == 1)
			{
				zeal.jQuery('#online-withdra').show();
				zeal.jQuery('#cheq-withdra').hide();
			}
			else if(wType == 2)
			{
				zeal.jQuery('#cheq-withdra').show();	
				zeal.jQuery('#online-withdra').hide();
			}
		},
		prooftype: function(type){
			if(type == 1)
			{
				var idproofval = $("#proof-select option:selected").val();
				if(idproofval != 0)
				{
					var idproof = $("#proof-select option:selected").text();
					zeal.jQuery('.idvalue').show();
					zeal.jQuery('#proof-name').html(idproof+' number*');
				}
			}
			else if(type == 2)
			{
				var idproofval = $("#chqproof-select option:selected").val();
				if(idproofval != 0)
				{	
					var idproof = $("#chqproof-select option:selected").text();
					zeal.jQuery('.idvalue').show();
					zeal.jQuery('#chqproof-name').html(idproof+' number*');
				}
			}
			
		},
		getDetails: function(transtype){
			
			if(transtype == 1)
			{
				var onlinamt = document.getElementById('txtonlineamt').value;
				var onlinacname = document.getElementById('txtonlinename').value;
				var onlinaccno = document.getElementById('txtonlineaccno').value;
				var onlinbankname = document.getElementById('txtonlinebankname').value;
				var onlineifsccode = document.getElementById('txtonlineifsc').value;
				var onlinmob = document.getElementById('txtonlinemobile').value;
				
				if(onlinamt == 0)
				{
					alert("please enter the amount to withdraw");
					//zeal.jQuery('#txtonlineamt').val('').css('border', 'solid 2px #f00');
					return false;	
				}
				else if(onlinacname == 0)
				{
					alert("please enter your Name");
					//zeal.jQuery('#txtonlinename').val('').css('border', 'solid 2px #f00');
					return false;
				}
				else if(onlinaccno == 0)
				{
					alert("please enter your Account No");
					//zeal.jQuery('#txtonlineaccno').val('').css('border', 'solid 2px #f00');
					return false;
				}
				else if(onlinbankname == 0)
				{
					alert("please enter your Bank Name");
					//zeal.jQuery('#txtonlinebankname').val('').css('border', 'solid 2px #f00');
					return false;
				}
				else if(onlineifsccode == 0)
				{
				...
				else if(onlinmob == 0)
				{
					...
					//zeal.jQuery('#txtonlinemobile').val('').css('border', 'solid 2px #f00');
					return false;
				}
				zeal.jQuery('.transparent-container').fadeIn('slow');
				zeal.jQuery('#online-iddet').show();
			}
			else
			{
				var chequeamt = document.getElementById('wamount').value;	
				var chequeaddr = document.getElementById('address').value;	
				var chequename = document.getElementById('cheqname').value;
				var chequemob = document.getElementById('txtmobile').value;
				var chequecount = document.getElementById('txtCountry').value;
				var chequestat = document.getElementById('txtstate').value;
				var chequecity = document.getElementById('txtcity').value;
				var chequepin = document.getElementById('txtpincode').value;
				...
				else if(chequeaddr == 0)
				{	
					alert("Please esnter your address")
					return false;
				}		
				else if(chequemob == 0)
				{	
					alert("Please esnter your mobile number")
					return false;
				}	
				else if(chequecount == 0)
				{	
					alert("Please esnter your country")
					return false;
				}	
				else if(chequestat == 0)
				{	
					alert("Please esnter your state")
					return false;
				}	
				else if(chequecity == 0)
				{	
					alert("Please esnter your city")
					return false;
				}	
				else if(chequepin == 0)
				{	
					alert("Please esnter your pincode")
					return false;
				}				
				zeal.jQuery('.transparent-container').fadeIn('slow');
				zeal.jQuery('#cheque-iddet').show();
			}
...
zeal.jQuery(document).ready(function($)
{
	...
	zeal.jQuery('#signup').live('submit', function(){
	...
	zeal.jQuery('#login').live('submit', function(){
		
		var loginname = zeal.jQuery('#username').val();
		var loginpass = zeal.jQuery('#password').val();
		var remember = zeal.jQuery('#remember-me').prop('checked');
		var verifycode = zeal.jQuery('#verify-txt').val();
		if(loginname.length == 0)
		{
			zeal.jQuery('#username').parent().addClass('txt-login-input-error');
			error = true;
		}
		else
			zeal.jQuery('#username').parent().removeClass('txt-login-input-error');
		if(loginpass.length == 0)
		{
			zeal.jQuery('#password').parent().addClass('txt-login-input-error');
			error = true;
		}
		else
			zeal.jQuery('#password').parent().removeClass('txt-login-input-error');
		
		zeal.jQuery('#login').hide();
		zeal.jQuery('.loading').show();
		zeal.jQuery.ajax({
			type: 'POST',  
			url: 'index.php',
			...
			success: function(jsonData, textStatus, XMLHttpRequest){
				if(jsonData.hasErrors)
				{
					zeal.jQuery('#login').show();
					zeal.jQuery('.loading').hide();
					zeal.errors.showError(jsonData.errors);
					if(jsonData.type == 'captcha' || jsonData.type == 'code')
					{
						zeal.jQuery('#captcha-veri-container').show();
						zeal.jQuery('#login').css('height', '400px');
						zeal.jQuery('#verify-txt').val('').css('border', 'solid 2px #f00');
						zeal.user.refreshCaptcha();
						document.getElementById('verify-txt').val('').focus();
						return false;
					}
					if(jsonData.type == 'auth')
					{
						...
						zeal.jQuery('#password').parent().addClass('txt-login-input-error');
						zeal.user.refreshCaptcha();
						zeal.jQuery('#verify-txt').val('').css('border', 'solid 2px #CB3536');
