...
<?php
include('sidebar.php');
require('classes/prizeSplit.php');
$tid=$_REQUEST['tid'];
	...
	while($result1=mysql_fetch_array($result))
	{
		...
		//$split['user_id'] = $result1['user_id'];
		//$split['tournament_id'] = $result1['tournament_id'];
		$split1[$count]['runs'] = $result1['run'];
		$split2[$count]['user_id'] = $result1['user_id'];
		$split3[$count]['tournament_id'] = $result1['tournament_id'];
		
		$count++;
	}

	
	$splitprize = new priSep($split1,$split2,$split3,$prizes,$oldcoin);
