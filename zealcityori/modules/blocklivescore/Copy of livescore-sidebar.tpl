...
<div class="live-score-widjet">
	<div class="live-score-menu rightscore">
		<ul>
			{if $homewinners}
			<li class="active" onclick="zeal.index.showContent('match-score', 'livescore', 4, this, 'rightscore')">Live Tournament</li>
			{/if}
			{if $liveScores}
	...
	{if $homewinners}
	<div class="match-score" id="livescore4">
		<div class="sidebar-current-tour">
			<div class="sidebar-current-tour-head">
				<div class="large">Name</div>
				<div class="small">Run</div>
				<div class="small">Prize</div>
			</div>		
			{foreach from=$homewinners item=homewinner name=homewinners}
			<div class="sidebar-current-tour-body">
				<div class="large">{$homewinner.username}</div>
				<div class="small">{$homewinner.run}</div>
				<div class="small">{$homewinner.prize}</div>
			</div>
			{/foreach}
		</div>
	</div>
	{/if}
	{if $liveScores}
	<div class="match-score hide" id="livescore1">
		{foreach from=$liveScores item=liveScore name=liveScores}
			<a href="http://www.cricville.com/fullscore/?id={$liveScore->match->id}" target="_blank">
                <div class="match-score-indivi{if ($smarty.foreach.liveScores.iteration >1)} hide{/if}" id="livescoreindi{$smarty.foreach.liveScores.iteration}">
                    ...
                    <div class="venue">{$liveScore->batting->teamname} Vs {$liveScore->bowling->teamname} {$liveScore->match->type|upper} series, {$liveScore->match->match_name} at</div>
	 ...
	 <div class="match-score hide" id="livescore3">
		...
		<div class="index-fixtures-indi">
			...
			<div class="index-fixtures-right">
				...
				{$upcomingMatch.match_date|date_format:'%B'} {$upcomingMatch.match_date|date_format:'%e'} - 
				{if $resultMatch->match->won_team_id == $resultMatch->batting->id}
					{$resultMatch->batting->team_nickname}
				{else}
					{$resultMatch->bowling->team_nickname}
				{/if}
				 won the match
			</div>
		</div>
