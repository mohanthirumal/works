...
if(isset($_REQUEST['action']) && strlen($_REQUEST['action']) && strtoupper($_REQUEST['action']) == 'FEE')
{
	...
	$winnercoin = $_REQUEST['txtwinnercoin'];
	$participatecoin = $_REQUEST['txtparticipatecoin'];
	$usertype = $_REQUEST['txtusertype'];
	
	$sql = "INSERT INTO coc_entry_fee(amount,winner_coin,participent_coin,user_type) VALUES('".$amount."', '".$winnercoin."', '".$participatecoin."', '".$usertype."');";
	mysql_query($sql);
...
if(isset($_REQUEST['action']) && strlen($_REQUEST['action']) && strtoupper($_REQUEST['action']) == 'FEEEDIT')
{
	...
	$id = $_REQUEST['id'];
	$sql = "UPDATE coc_entry_fee SET amount = '".$amount."', winner_coin='".$winnercoin."', participent_coin='".$participatecoin."', user_type='".$usertype."' WHERE id = '".$id."'";
	mysql_query($sql);
	header('Location: entryfee.php');
}
...
if(isset($_REQUEST['action']) && strlen($_REQUEST['action']) && strtoupper($_REQUEST['action']) == 'TOURNAMENTEDIT')
{
	...
	$sql = "UPDATE coc_tournament_prize SET `prize` = '".$_REQUEST['txtprizepool']."' WHERE `tournament_id` = '".$id."' ";
	mysql_query($sql);
...
if(isset($_REQUEST['action']) && strlen($_REQUEST['action']) && strtoupper($_REQUEST['action']) == 'DELETEPERIODTOURNAMENT')
{
	$id = $_REQUEST['id'];
	$sql = "DELETE FROM coc_period_tournament WHERE id = ".$id;
	mysql_query($sql);
	header('Location: period-tournament.php');
}
