	...
	<form method="post" action="process.php" enctype="multipart/form-data">
		<table align="center" width="450px" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				...
				<td>
                	<?php
						$sql = "SELECT * FROM coc_tournament_prize WHERE `tournament_id` = '".$resulta['id']."'";
						$resultResource = mysql_query($sql);
						$result = mysql_fetch_array($resultResource)
					?>
					<input type="text" name="txtprizepool" id="txtprizepool1" value="<?php echo $result['prize']; ?>" onchange="calculatePrize();">
				...
				<td>
					<select name="match">
						...
						$sql = "SELECT md.id AS id, t1.teamname AS team1, t2.teamname AS team2, md.match_date,match_name FROM match_details md
								...
								INNER JOIN team_details t2 ON t2.id = md.team2 where md.display_status = 'future' ORDER BY (md.id ='".$resulta['match_id']."') DESC, md.id";    
						$resultResource = mysql_query($sql);
						while($result = mysql_fetch_array($resultResource))
						{
			...
			<tr>
				...
				<td>
					<select name="status">
						...
						<option value="Future"<?php echo ($resulta['status'] == 'Future')?' selected':''; ?>>Future</option>
...
//						}
				...
				<td>
					<select name="match">
						...
						$sql = "SELECT md.id AS id, t1.teamname AS team1, t2.teamname AS team2, match_date,match_name FROM match_details md
								...
								where md.display_status = 'future'" ;    
						$resultResource = mysql_query($sql);
						while($result = mysql_fetch_array($resultResource))
						{
							echo '<option value="'.$result['id'].'">'.date('d-m-Y', $result['match_date']).' - '.$result['team1'].' Vs '.$result['team2'].' - '.$result['match_name'].'</option>';   //
    ...
    <?php
		if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'filter')
		{
			...
			$_SESSION['status'] = $_POST['filterstatus'];
			$_SESSION['endtime'] = $_POST['datefilter'];
			$enddate = strtotime($_SESSION['endtime']);
    ...
    <form action="tournament.php" method="post">
                ...
                <select name="filtermatch">
                    ...
                    $sql = "SELECT md.id AS id, t1.teamname AS team1, t2.teamname AS team2, match_date,match_name FROM match_details md
                            INNER JOIN team_details t1 ON t1.id = md.team1
                            INNER JOIN team_details t2 ON t2.id = md.team2 ORDER BY md.id DESC" ;
                    $resultResource = mysql_query($sql);
					while($result = mysql_fetch_array($resultResource))
                    {
                        echo '<option value="'.$result['id'].'">'.date('d-m-Y', $result['match_date']).' - '.$result['team1'].' Vs '.$result['team2'].' - '.$result['match_name'].'</option>';   
 ...
 <?php
		if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'future-status')
		{
			$sql = 'UPDATE `coc_tournament` SET `status` = "Open" WHERE `match_id` = '.$_REQUEST['matchstatus'];
			mysql_query($sql);
		}
	?>
     <div style="clear:both"></div>
    <form id="tour_list" name="tour_list" method="post" onsubmit="return chkStatus();">
    	<br />
        <select name="matchstatus" id="matchstatus" >
            <option value="0">---Select---</option>
            <?php
            $sql = "SELECT md.id AS id, t1.teamname AS team1, t2.teamname AS team2, match_date,match_name FROM match_details md
                    INNER JOIN team_details t1 ON t1.id = md.team1
                    INNER JOIN team_details t2 ON t2.id = md.team2 WHERE md.display_status = 'future' ORDER BY md.id ASC" ;
            $resultResource = mysql_query($sql);
            while($result = mysql_fetch_array($resultResource))
            {
                echo '<option value="'.$result['id'].'">'.date('d-m-Y', $result['match_date']).' - '.$result['team1'].' Vs '.$result['team2'].' - '.$result['match_name'].'</option>';   
        ...
        <input type="submit" id="tour_submit" name="tour_submit" value="Change Status" />
        <input type="hidden" name="action" value="future-status"/>
	 </form>
	<table width="80%" class="list" border="1" cellpadding="5px" cellspacing="0" align="center" style="margin-top:20px;">
		...
		<?php
			...
			$sql =	"select *,t.id as id,t.name as tname,k.percentage as kitty,fee.amount as amt,
					...
					p.player as player,pp.prize as prize,ttype.name as type,t1.team_nickname as team1,t2.team_nickname as team2 from coc_tournament t
					...
					inner join coc_tournament_prize pp on pp.tournament_id=t.id
			...
			if((isset($_SESSION['match_id'])) && ($_SESSION['match_id'] > 0))
				$sql .= "where t.match_id='".$_SESSION['match_id']."' order by t.id desc";
			else if((isset($_SESSION['status'])) )
			{
				$sql .= 'where t.status="'.$_SESSION['status'].'" AND t.endtime  BETWEEN "'.$date.'" AND "'.$next_day.'" order by t.id desc';
			}
			else
			...
			while($result = mysql_fetch_array($resultResource))
			{
				...
				<tr class='.(($count % 2 == 0)?"even":"odd").'>
					...
					<td><a class="list-action-btn" href="joined-users-list.php?id='.$result['id'].'&type=2" target="_blank">Joined Users</a></td>
				</tr>
				';
				$count++;
			}
	...
	function chkStatus()
	{
		...
		if(confirm("Are you sure to change Status for " + StatusText))
			return true;
		else
			return false;
	}
