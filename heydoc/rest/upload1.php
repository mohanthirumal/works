		...
		while($x<=$excel->sheets[0]['numRows'])
		{
		  ...
		  $json .= '{
			...
			if(isset($excel->sheets[0]['cells'][$x][1]))
				$json .= '"specialist":"'.$excel->sheets[0]['cells'][$x][1].'",';
			if(isset($excel->sheets[0]['cells'][$x][2]))
				$json .= '"name":"'.$excel->sheets[0]['cells'][$x][2].'",';
			if(isset($excel->sheets[0]['cells'][$x][3]))
				$json .= '"phone":"'.$excel->sheets[0]['cells'][$x][3].'",';
			if(isset($excel->sheets[0]['cells'][$x][4]))
				$json .= '"area":"'.$excel->sheets[0]['cells'][$x][4].'",';
			if(isset($excel->sheets[0]['cells'][$x][5]))
				$json .= '"address":"'.$excel->sheets[0]['cells'][$x][5].'",';
			if(isset($excel->sheets[0]['cells'][$x][6]) && strlen($excel->sheets[0]['cells'][$x][6]) > 0 && isset($excel->sheets[0]['cells'][$x][7]))
				$json .= '"location":{"__type": "GeoPoint", "longitude": '.$excel->sheets[0]['cells'][$x][6].', "latitude": '.$excel->sheets[0]['cells'][$x][7].'},';
			if(isset($excel->sheets[0]['cells'][$x][8]))
				$json .= '"city":"'.$excel->sheets[0]['cells'][$x][8].'",';
